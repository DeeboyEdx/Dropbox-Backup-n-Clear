...
function Free-File-Sync {
    ...
    Begin {
        ...
        if (-not (Test-Path $ffs)) {
            Write-Host "FreeFileSync not installed." -ForegroundColor Red
        ...
        switch ($PSCmdlet.ParameterSetName) {
            ...
            'BuildAJob' 
            {
                ...
                $Sync_Was_Successful = Free-File-Sync -JobsPath $env:TEMP -Filter $jobFileNameS -ShowLogs:$ShowLogs -Verbose:$VerbosePreference -ReturnBoolean:$ReturnBoolean
            }
        }
    }
    ...
    Process {
        ...
        foreach ($job in $sync_Jobs) {
            ...
            $completion_message = switch ($_process.ExitCode) {
                0 { "Sync completed successfully"; $message_color = "Green"; $Sync_Was_Successful = $True }
